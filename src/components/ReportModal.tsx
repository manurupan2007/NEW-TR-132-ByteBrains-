...
export default function ReportModal({ isOpen, onOpenChange, result }: ReportModalProps) {
  ...
  const handleExport = () => {
    ...
    downloadAnchorNode.setAttribute("download", `sentinel_clinical_report_${new Date().getTime()}.json`);
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-3xl bg-white text-black p-0 overflow-hidden border-black border-4 rounded-none shadow-[10px_10px_0px_0px_rgba(0,0,0,1)] print:shadow-none print:border-none print:w-full print:max-w-none print:h-auto">
        <div className="bg-black p-4 flex justify-between items-center print:bg-zinc-100 print:text-black print:border-b-2 print:border-black">
          <div className="flex items-center gap-4">
            <div className="w-10 h-10 bg-white flex items-center justify-center p-1 border-black border">
               <div className="flex flex-col items-center justify-center border border-black w-full h-full">
                 <div className="text-[5px] font-black leading-none text-black">TENSOR</div>
                 <div className="w-full h-[1px] bg-black my-0.5" />
                 <div className="text-[6px] font-bold text-black">BYTE</div>
               </div>
            </div>
            <span className="text-white font-mono text-xs font-bold tracking-[0.3em] print:text-black">MISSION // PS49</span>
          </div>
          <div className="flex items-center gap-4">
             <Badge variant="outline" className="text-white border-white rounded-none font-mono text-[8px] uppercase print:text-black print:border-black">Highly Confidential</Badge>
             <div className="text-orange-500 font-black text-xl print:text-orange-600">T '26</div>
          </div>
        </div>

        <div className="p-8 space-y-8 font-serif leading-relaxed h-[80vh] overflow-y-auto custom-scrollbar print:h-auto print:overflow-visible print:p-0 print:m-0">
          ...
          <div className="space-y-4">
            <h3 className="font-black uppercase tracking-[0.2em] text-xs bg-black text-white px-3 py-1 inline-block print:bg-zinc-200 print:text-black">Diagnostic Synthesis</h3>
        ...
        <div className="p-6 bg-zinc-200 border-t-4 border-black grid grid-cols-2 gap-4 print:hidden">
          <Button 
            variant="outline" 
            onClick={handleExport}
            className="border-2 border-black text-black hover:bg-black hover:text-white rounded-none font-black uppercase tracking-widest text-xs h-12 shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] active:shadow-none active:translate-x-1 active:translate-y-1 transition-all"
          >
          ...
          <Button 
            onClick={handlePrint}
            className="bg-orange-600 text-white border-2 border-black hover:bg-orange-700 rounded-none font-black uppercase tracking-widest text-xs h-12 shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] active:shadow-none active:translate-x-1 active:translate-y-1 transition-all"
          >
